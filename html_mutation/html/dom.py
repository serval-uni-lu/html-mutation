...
class DomTree:
    ...
    def __getstate__(self) -> dict:
        ...
        dict["tree"] = html.tostring(
            self.tree, encoding="unicode", method="html"
        )
        return dict
    ...
    def find_by_tag(self, tags: set[Tag] | Tag) -> list[Element]:
        if isinstance(tags, Tag):
            ...
            query = ".//*[{}]".format(
                " or ".join("self::" + tag.value for tag in tags)
            )
...
def parse(dom_content: str) -> DomTree:
    return DomTree(
        html5lib.parse(
            dom_content, treebuilder="lxml", namespaceHTMLElements=False
        )
    )
...
class DomInfo:
    def __init__(
        ...
        self.dom = dom
        self.image = image
        self.dom_id = dom_id if dom_id else "<NONE>"
